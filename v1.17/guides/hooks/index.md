...
- **Matcher portability** - `matcher` filters by target-native tool names (Factory `Execute`, GitHub Copilot tool names, Claude `Edit|Write`). A matcher that works on one target may match nothing on another. See [@hooks](https://getpromptscript.dev/v1.17/reference/language/#hooks).
- **Terminal commands** - use `pre-terminal-command` instead of a broad `pre-tool-use` matcher. PromptScript supplies audited native tool defaults, allows `targets.<name>.matcher` overrides, and reports `PS4002` for best-effort or unsupported host coverage. See [Terminal command semantics](https://getpromptscript.dev/v1.17/features/automation/#terminal-command-semantics).
...
See [`prs compile`](https://getpromptscript.dev/v1.17/reference/cli/#prs-compile) for full watch options.
