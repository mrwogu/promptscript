...
| Goal                                       | Reference                                                                                                          |
| ------------------------------------------ | ------------------------------------------------------------------------------------------------------------------ |
| Understand a complete `.prs` file          | [File Anatomy](https://getpromptscript.dev/v1.17/reference/language/file-anatomy/index.md)                         |
| Choose valid block body shapes             | [Values and Block Bodies](https://getpromptscript.dev/v1.17/reference/language/values-and-block-bodies/index.md)   |
| Predict inheritance and import results     | [Composition and Precedence](https://getpromptscript.dev/v1.17/reference/language/composition/index.md)            |
| Understand declaration order               | [Execution Order](https://getpromptscript.dev/v1.17/reference/language/execution-order/index.md)                   |
| Choose `@extend`, `field!`, or `@override` | [Merge and Replacement](https://getpromptscript.dev/v1.17/reference/language/merge-and-replacement/index.md)       |
| Customize generated headings               | [Section Headers](https://getpromptscript.dev/v1.17/reference/language/section-headers/index.md)                   |
| Upgrade syntax and resolve diagnostics     | [Versions and Diagnostics](https://getpromptscript.dev/v1.17/reference/language/versions-and-diagnostics/index.md) |
...
For incompatible block shapes, the existing target body wins. Under syntax `1.5.0`, later declarations can modify the merged result, so a local block, `@extend`, or `@override` placed after `@use` can become the final value. See [Composition and Precedence](https://getpromptscript.dev/v1.17/reference/language/composition/index.md) for the normative matrix.
...
See the [Examples guide](https://getpromptscript.dev/v1.17/guides/examples/index.md) for a full walkthrough.
...
Every built-in target has an explicit hook capability classification. Targets without native project hooks and modes that cannot emit additional files report `PS4002` with a fallback such as `prs compile --watch`. See [Hooks and Workflows](https://getpromptscript.dev/v1.17/features/automation/#hook-capability-matrix) for the complete 48-target matrix and project-root behavior.
...
**Target Support:** The `@mcpServers` block is emitted to target-native MCP config files. See [Configuration Reference](https://getpromptscript.dev/v1.17/reference/config/#mcp-hooks-plugins-support) for the full list of supported targets and their output paths.
...
These come from the resolver, not the validator (`PS0XX` rules). They appear during `prs compile`, not `prs validate`. See the [Skill Overlays Guide](https://getpromptscript.dev/v1.17/guides/skill-overlays/index.md) for examples and remediation.
...
See [Registry Aliases](https://getpromptscript.dev/v1.17/guides/registry/#registry-aliases) for alias configuration.
...
See [Block Shapes](https://getpromptscript.dev/v1.17/reference/block-shapes/index.md) for the canonical shape, compatibility forms, merge rules, diagnostics, and formatter behavior of every built-in block.
