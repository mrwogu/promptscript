...
See the [Examples guide](https://getpromptscript.dev/v1.15/guides/examples/index.md) for a full walkthrough.
...
**Target Support:** The `@mcpServers` block is emitted to target-native MCP config files. See [Configuration Reference](https://getpromptscript.dev/v1.15/reference/config/#mcp-hooks-plugins-support) for the full list of supported targets and their output paths.
...
These come from the resolver, not the validator (`PS0XX` rules). They appear during `prs compile`, not `prs validate`. See the [Skill Overlays Guide](https://getpromptscript.dev/v1.15/guides/skill-overlays/index.md) for examples and remediation.
...
See [Registry Aliases](https://getpromptscript.dev/v1.15/guides/registry/#registry-aliases) for alias configuration.
