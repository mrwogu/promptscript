...
Fields a target cannot represent are reported with `PS4003` compatibility warnings (agent name, field, target, and the targets that do support the field) instead of being dropped silently. See the [Field Support Matrix](https://getpromptscript.dev/v1.19/features/agents/#field-support-matrix) for the authoritative per-target statuses.
...
- [Agent Platform - Agents](https://getpromptscript.dev/v1.19/features/agents/index.md)
- [Skills & Local Example](https://getpromptscript.dev/v1.19/examples/skills-and-local/index.md)
