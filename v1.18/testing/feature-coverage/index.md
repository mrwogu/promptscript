...
The target catalog contains 49 formatters. The matrix currently classifies 37 of them. A missing entry means the target has not been classified for that feature, not that the feature is unsupported. The formatter registry and generated [platform matrix](https://getpromptscript.dev/v1.18/reference/formatters/index.md) remain the source of truth for target availability.
