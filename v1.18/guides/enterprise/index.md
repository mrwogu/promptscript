...
This allows `prs init --yes` to automatically connect to the company registry without manual configuration. See [User-Level Configuration](https://getpromptscript.dev/v1.18/guides/user-config/index.md) for details.
...
Advance when all pilot repositories pass `prs validate --strict`, generated output drift is zero, at least 95% of pilot CI runs pass for five business days, and every reported P0 or P1 migration issue is closed.
...
Advance when every onboarded team has an owner and rollback contact, at least 90% of in-scope repositories compile from committed sources in CI, and no open P0 or P1 incident is attributable to generated instructions.
...
Review monthly: validation pass rate, output-drift rate, mean time to resolve PromptScript incidents, unsupported target warnings, repository adoption, and rollback exercises. Pause new onboarding when a P0 remains open or the rolling seven-day validation pass rate drops below 95%.
...
Registry aliases do not carry credentials. For SSO-gated repositories, configure authentication under `registry.git.auth` for the default registry, or expose the host credential through the Git credential helper. Store service account tokens or deploy keys in a secrets manager.
