...
`id`, `syntax`, and `targets` are required even when `input.entry` uses its default. See the [Policy Engine guide](https://getpromptscript.dev/v1.14/guides/policy-engine/index.md) for `policies` syntax.
...
PromptScript ships 48 built-in targets. See [Target Platforms](https://getpromptscript.dev/v1.14/features/target-platforms/index.md) for platform families and [Supported Formatters](https://getpromptscript.dev/v1.14/reference/formatters/index.md) for the full capability matrix.
...
Multi-line `@shortcuts` are automatically converted to executable slash commands in `.cursor/commands/`. Type `/` in Cursor chat (1.6+) to invoke them. See [Language Reference](https://getpromptscript.dev/v1.14/reference/language/#cursor-slash-commands-16).
...
See [Formatters API](https://getpromptscript.dev/v1.14/api-reference/formatters/src/index.md) for more details.
