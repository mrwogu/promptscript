...
operations. Inherited files, top-level imports, inline composed skills, and
inline `@use` declarations carried by `@extend` bodies are inspected before
resolution. If any reachable source declares syntax `1.5.0` or uses an ordered
operation feature, the composed graph uses declaration order. A lower-version
entry file remains valid, but PS018 reports the ordered-operation requirement
