...
const PROMPTSCRIPT_YAML_MARKER = '# promptscript-generated: 2024-01-01T00:00:00.000Z';
...
describe('compile command - overwrite protection', () => {
  ...
  describe('when file exists with PromptScript marker', () => {
    ...
    it('should merge user-owned hook files in non-interactive mode', async () => {
      const path = '.github/hooks/promptscript.json';
      const generated = JSON.stringify({
        version: 1,
        hooks: {
          preToolUse: [{ type: 'command', bash: 'echo generated' }],
        },
      });
      const outputs = new Map([[path, createMockOutput(path, generated)]]);
      ...
      mockReadFile.mockResolvedValue(
        JSON.stringify({
          version: 1,
          hooks: {
            preToolUse: [{ type: 'command', bash: 'echo user-owned' }],
          },
        })
      );
      ...
      expect(JSON.parse(written)).toEqual({
        ...
        hooks: {
          preToolUse: [
            { type: 'command', bash: 'echo user-owned' },
            { type: 'command', bash: 'echo generated' },
          ],
        },
      });
    });
...
`);
    ...
    it('should replace empty Codex config after removing owned hooks', async () => {
      ...
      mockIsTTY.mockReturnValue(false);
      mockExistsSync.mockReturnValue(true);
      mockReadFile.mockResolvedValue(`[[hooks.PreToolUse]]
[[hooks.PreToolUse.hooks]]
type = "command"
command = "echo old # promptscript-generated:owned"
`);
      ...
      expect(process.exitCode).toBeUndefined();
      expect(mockPrompts.select).not.toHaveBeenCalled();
      expect(mockWriteFile).toHaveBeenCalledWith(resolve(path), 'max_threads = 8\n', 'utf-8');
    });
  });
  ...
  describe('YAML marker detection', () => {
    it('should detect YAML comment marker in frontmatter files', async () => {
      const outputs = new Map([
        [
          '.factory/skills/commit/SKILL.md',
          createMockOutput('.factory/skills/commit/SKILL.md', 'new content'),
        ],
      ]);
      ...
      // File contains YAML comment marker inside frontmatter
      mockReadFile.mockResolvedValue(
        `---\n${PROMPTSCRIPT_YAML_MARKER}\nname: commit\ndescription: Create git commits\n---\n\nOld content`
      );
  ...
  describe('--cwd option', () => {
    ...
    it('should fail when entry file not found in cwd directory', async () => {
      ...
      await compileCommand({ cwd: '/empty/project' }, mockServices);
      expect(process.exitCode).toBe(1);
    });
    ...
    it('should fall back to cwd when no config files found', async () => {
      ...
      await compileCommand({ cwd: '/nonexistent' }, mockServices);
      expect(process.exitCode).toBe(1);
    });
  });
});
