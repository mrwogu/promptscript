...
describe('cleanupManagedOutputs', () => {
  ...
  it('should remove an obsolete generated OpenCode plugin', async () => {
    const project = await createTemporaryDirectory('promptscript-cleanup-opencode-plugin-');
    const currentPlugin = join(project, '.opencode', 'plugins', 'promptscript.ts');
    const legacyPlugin = join(project, '.opencode', 'plugins', 'legacy-promptscript.ts');
    await mkdir(dirname(currentPlugin), { recursive: true });
    await writeFile(
      currentPlugin,
      ...
      [
        'OPENCODE.md',
        {
          ...
          managedOutputFiles: [
            ...
            '.opencode/plugins/legacy-promptscript.ts',
          ],
        },
      ],
    ]);
    ...
    expect(result.removed).toEqual([legacyPlugin, currentPlugin]);
    await expect(readFile(currentPlugin, 'utf-8')).rejects.toMatchObject({ code: 'ENOENT' });
    await expect(readFile(legacyPlugin, 'utf-8')).rejects.toMatchObject({ code: 'ENOENT' });
  });
