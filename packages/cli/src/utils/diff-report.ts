...
function normalizeWarning(
  ...
  return {
    ...
    ...(normalizeLocation(warning.location, projectRoot)
      ? { location: normalizeLocation(warning.location, projectRoot) }
      : {}),
  };
}
...
function normalizeError(
  ...
  return {
    ...
    ...(normalizeLocation(error.location, projectRoot)
      ? { location: normalizeLocation(error.location, projectRoot) }
      : {}),
  };
}
...
function isOwnedOutput(path: string, content: string): boolean {
  const hasLegacyMarker = content
    .split(/\r?\n/)
    .slice(0, 50)
    .some((line) => line.trimStart().startsWith('> Auto-generated by PromptScript'));
  return (
    isGeneratedByPromptScript(content) ||
    hasLegacyMarker ||
    isPromptScriptOwnedHookOutput(path, content)
  );
}
