...
// Shared marker-stripping utility (used by both compile and diff)
  ...
  async function safeWrite(path: string, content: string): Promise<boolean> {
    try {
      ...
      if (code === 'ENOTDIR' || code === 'EEXIST') {
        const msg =
          code === 'ENOTDIR'
            ? `Cannot create directory '${dirname(path)}' because a parent path already exists as a file. Remove the file or disable this target.`
            : `Cannot write '${path}' because a directory exists at that path.`;
