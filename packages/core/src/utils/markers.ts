...
const TYPESCRIPT_MARKER = '//\\s*promptscript-generated:';
const LEGACY_TYPESCRIPT_MARKER = '//\\s*Generated by PromptScript - do not edit\\.';
...
export function stripPromptScriptMarkers(content: string): string {
  return content
    ...
    .replace(new RegExp(`${TYPESCRIPT_MARKER}[^\\n]+\\n*`, 'g'), '')
    .replace(new RegExp(`${LEGACY_TYPESCRIPT_MARKER}\\n*`, 'g'), '')
...
export function isGeneratedByPromptScript(content: string): boolean {
  const head = new RegExp(
    `^\\s*(?:${YAML_MARKER}|${TYPESCRIPT_MARKER}|${LEGACY_TYPESCRIPT_MARKER}|${HTML_MARKER})`
  );
