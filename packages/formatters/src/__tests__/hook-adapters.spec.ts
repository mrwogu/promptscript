...
interface GeneratedPluginHooks {
  'tool.execute.before': (input: GeneratedHookInput, output: GeneratedHookOutput) => Promise<void>;
  'tool.execute.after': (input: GeneratedHookInput, output: GeneratedHookOutput) => Promise<void>;
}

interface GeneratedSubprocess {
  stdin: {
...
type GeneratedPluginFactory = (context: { directory: string }) => Promise<GeneratedPluginHooks>;

async function loadGeneratedPlugin(
  source: string,
  spawn: (options: unknown) => GeneratedSubprocess
): Promise<GeneratedPluginHooks> {
  ...
  const factory = exportsObject['PromptScriptHooks'];
  if (typeof factory !== 'function') {
    throw new Error('Generated OpenCode plugin did not export PromptScriptHooks');
  }
  return (factory as GeneratedPluginFactory)({ directory: '/repo' });
}
...
describe('hook-adapters', () => {
  ...
  describe('generateOpenCodePlugin', () => {
    ...
    it('resolves portable scripts from a nested hook cwd', () => {
      const hooks = extractHooks(
        makeHooksBlock({
          validate: {
            event: 'pre-tool-use',
      ...
      expect(plugin).toContain(
        '"command":["python3","../../.promptscript/scripts/check.py"],"cwd":"tools/hooks"'
      );
    });
    ...
    it('produces a self-contained deterministic plugin', () => {
      ...
      expect(plugin).toMatch(/^\/\/ promptscript-generated: opencode-plugin\n/);
      ...
      expect(plugin).toContain('buildPayload(entry.rule, input, output.args)');
      expect(plugin).toContain('buildPayload(entry.rule, input, input.args, result)');
      ...
      expect(plugin).toContain('new TextEncoder().encode(value).byteLength');
      expect(plugin).toContain("args: '[truncated]'");
      expect(plugin).toContain("return '{}';");
      // Timeout is always bounded and escalates when graceful termination fails.
      expect(plugin).toContain('const DEFAULT_TIMEOUT_MS = 30000;');
      expect(plugin).toContain("proc.kill('SIGTERM')");
      expect(plugin).toContain("proc.kill('SIGKILL')");
      // Hook processes run asynchronously and report nonzero exits.
      expect(plugin).toContain('void runRule(');
      expect(plugin).toContain('const exitCode = await proc.exited;');
      expect(plugin).toContain('exited with code');
      // Deterministic recompilation.
      expect(generateOpenCodePlugin(hooks)).toBe(plugin);
    });

    it('bounds multibyte payloads and reads post-hook args from input', async () => {
      const plugin = generateOpenCodePlugin([
        { id: 'capture', event: 'post-tool-use', command: ['prs', 'capture'] },
      ])!;
      let payload = '';
      const handlers = await loadGeneratedPlugin(plugin, () => ({
        stdin: {
          write(value): void {
            payload = value;
          },
      ...
      await handlers['tool.execute.after'](
        {
          ...
          sessionID: 'session',
          callID: 'call',
          args: { content: '🙂'.repeat(20_000) },
        },
        { title: 'Edited', output: 'ok', metadata: {} }
      );
      await Promise.resolve();

      expect(Buffer.byteLength(payload, 'utf8')).toBeLessThanOrEqual(32_768);
      expect(JSON.parse(payload)).toMatchObject({
        args: '[truncated]',
        result: { title: 'Edited', output: 'ok', metadata: {} },
      });
    });

    it('returns before hook exit and escalates the default timeout', async () => {
      vi.useFakeTimers();
      ...
      const plugin = generateOpenCodePlugin([
        { id: 'check', event: 'pre-tool-use', command: ['node', 'check.mjs'] },
      ])!;
      const handlers = await loadGeneratedPlugin(plugin, () => ({
      ...
      await Promise.resolve();
      await Promise.resolve();
      expect(error).toHaveBeenCalledWith('[promptscript] opencode hook "check" exited with code 1');
    });
    ...
    it('documents the OpenCode coverage limits in the generated header', () => {
      ...
      expect(plugin).toContain('do not expose model or agent context');
    });
  });
});
