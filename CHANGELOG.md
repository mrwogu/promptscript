...
- **Antigravity formatter** - Fixed default output path from `.junie/guidelines.md` to `.agent/rules/project.md` to match the actual formatter implementation

### Changed

- **Type definitions** - Removed `jetbrains` and `gemini` from `TargetName` type (not yet implemented)
- **README diagram** - Updated to show only currently supported targets: GitHub Copilot, Claude Code, Cursor, Antigravity
- **CLI init command** - Added Antigravity to AI tool detection and target suggestions
