...
See the [Policy Engine guide](https://getpromptscript.dev/dev/guides/policy-engine/index.md) for `policies` syntax.
...
Multi-line `@shortcuts` are automatically converted to executable slash commands in `.cursor/commands/`. Type `/` in Cursor chat (1.6+) to invoke them. See [Language Reference](https://getpromptscript.dev/dev/reference/language/#cursor-slash-commands-16).
...
- `opus`, `sonnet`, `haiku`, and `fable` are floating aliases. Each one points at the newest release of its Claude family that is not retired, listed in [Floating Aliases](https://getpromptscript.dev/dev/reference/models/#floating-aliases).
...
The [Model Catalog](https://getpromptscript.dev/dev/reference/models/#target-model-names) shows what each target writes for common values.
