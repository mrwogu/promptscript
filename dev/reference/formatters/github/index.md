...
- Agent `model` and `specModel` values become Copilot model names (display names such as `Claude Sonnet 4.5`); `inherit` is omitted, `auto` becomes `Auto`, and names outside the catalog pass through unchanged (see [Model Catalog](https://getpromptscript.dev/dev/reference/models/#target-model-names))
