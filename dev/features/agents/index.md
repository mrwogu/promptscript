...
The [Model Catalog](https://getpromptscript.dev/dev/reference/models/index.md) lists every built-in model, the current release behind each floating alias, and what each target writes. See [Configuration: models](https://getpromptscript.dev/dev/reference/config/#models) for adding models that are not in the built-in catalog yet.
