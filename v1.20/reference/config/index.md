...
See the [Policy Engine guide](https://getpromptscript.dev/v1.20/guides/policy-engine/index.md) for `policies` syntax.
...
Project and user opt-outs are hard vetoes. Environment opt-in cannot override them. See [Anonymous Usage Telemetry](https://getpromptscript.dev/v1.20/reference/telemetry/index.md) for collected fields, delivery behavior, and all opt-out controls.
...
PromptScript ships 50 built-in targets. See [Target Platforms](https://getpromptscript.dev/v1.20/features/target-platforms/index.md) for platform families and [Supported Formatters](https://getpromptscript.dev/v1.20/reference/formatters/index.md) for the full capability matrix.
...
Multi-line `@shortcuts` are automatically converted to executable slash commands in `.cursor/commands/`. Type `/` in Cursor chat (1.6+) to invoke them. See [Language Reference](https://getpromptscript.dev/v1.20/reference/language/#cursor-slash-commands-16).
...
See [Formatters API](https://getpromptscript.dev/v1.20/api-reference/formatters/src/index.md) for more details.
